YUI.add('moodle-enrol_ishinemanual-quickenrolment', function(Y) {
    ...
    Y.extend(USERENROLLER, Y.Base, {
        ...
        initializer : function(config) {
                ...
                .append(create('<div class="'+CSS.WRAP+' modal show modal-dialog modal-content"></div>')
                    ...
                    .append(create('<div class="'+CSS.CONTENT+' modal-body"></div>')
                        .append(create('<div class="'+CSS.SEARCHCONTROLS+' form-inline"></div>')
                            .append(create('<div class="'+CSS.ENROLMENTOPTION+' '+CSS.ROLE+'"><label for="id_enrol_ishinemanual_assignable_roles">'+M.util.get_string('assignroles', 'role')+'</label></div>')
                                    .append(create('<select id="id_enrol_ishinemanual_assignable_roles" class="custom-select"><option value="">'+M.util.get_string('none', 'enrol')+'</option></select>'))
                            )
            ...
            Y.all('.enrol_ishinemanual_plugin input').each(function(node){
            ...
            if (this.get(UEP.COHORTSAVAILABLE)) {
                this.get(UEP.BASE).one('.'+CSS.ENTITYSELECTOR)
                    .append(create('<input type="radio" id="id_enrol_ishinemanual_entity_users" name="enrol_ishinemanual_entity" value="users" class="m-r-1" checked="checked"/>'))
                    .append(create('<label for="id_enrol_ishinemanual_entity_users">'+ M.util.get_string('browseusers', 'enrol_ishinemanual')+'</label>'))
                    .append(create('<input type="radio" id="id_enrol_ishinemanual_entity_cohorts" name="enrol_ishinemanual_entity" class="m-x-1" value="cohorts"/>'))
                    .append(create('<label for="id_enrol_ishinemanual_entity_cohorts">'+M.util.get_string('browsecohorts', 'enrol_ishinemanual')+'</label>'));
                this.get(UEP.BASE).one('#id_enrol_ishinemanual_entity_cohorts').on('change', this.search, this);
                this.get(UEP.BASE).one('#id_enrol_ishinemanual_entity_users').on('change', this.search, this);
            } else {
                this.get(UEP.BASE).one('.'+CSS.ENTITYSELECTOR)
                    .append(create('<input type="hidden" name="enrol_ishinemanual_entity" value="users"/>'));
            }
        ...
        populateAssignableRoles : function() {
            this.on('assignablerolesloaded', function(){
                ...
                Y.one('#id_enrol_ishinemanual_assignable_roles').focus();
            }, this);
        ...
        show : function(e) {
            ...
            var rolesselect = Y.one('#id_enrol_ishinemanual_assignable_roles');
            if (rolesselect) {
                rolesselect.focus();
            }
        },
        ...
        currentEntity : function() {
            var entity = CSS.USER;
            var cohortsinput = Y.one('#id_enrol_ishinemanual_entity_cohorts');
    ...
    M.enrol_ishinemanual = M.enrol_ishinemanual || {};
    M.enrol_ishinemanual.quickenrolment = {
        init : function(cfg) {
