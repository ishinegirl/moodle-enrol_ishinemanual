...
/**
 * iShineManual enrolment external functions.
 *
 * @package    enrol_ishinemanual
 * @category   external
 * @copyright  2011 Jerome Mouneyrac
 ...
 * @since Moodle 2.2
 */
class enrol_ishinemanual_external extends external_api {
    ...
    public static function enrol_users($enrolments) {
        ...
        // Retrieve the ishinemanual enrolment plugin.
        $enrol = enrol_get_plugin('ishinemanual');
        if (empty($enrol)) {
            throw new moodle_exception('ishinemanualpluginnotinstalled', 'enrol_ishinemanual');
        }
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            // Check that the user has the permission to ishinemanual enrol.
            ...
            if (!array_key_exists($enrolment['roleid'], $roles)) {
                ...
                throw new moodle_exception('wsusercannotassign', 'enrol_ishinemanual', '', $errorparams);
            }

            // Check ishinemanual enrolment plugin instance is enabled/exist.
            $instance = null;
            $enrolinstances = enrol_get_instances($enrolment['courseid'], true);
            foreach ($enrolinstances as $courseenrolinstance) {
              if ($courseenrolinstance->enrol == "ishinemanual") {
            ...
            if (empty($instance)) {
              $errorparams = new stdClass();
              $errorparams->courseid = $enrolment['courseid'];
              throw new moodle_exception('wsnoinstance', 'enrol_ishinemanual', $errorparams);
            }
            ...
            if (!$enrol->allow_enrol($instance)) {
                ...
                $errorparams->courseid = $enrolment['courseid'];
                $errorparams->userid = $enrolment['userid'];
                throw new moodle_exception('wscannotenrol', 'enrol_ishinemanual', '', $errorparams);
            }
    ...
    public static function unenrol_users($enrolments) {
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            $instance = $DB->get_record('enrol', array('courseid' => $enrolment['courseid'], 'enrol' => 'ishinemanual'));
            if (!$instance) {
                throw new moodle_exception('wsnoinstance', 'enrol_ishinemanual', $enrolment);
            }
            ...
            if (!$enrol->allow_unenrol($instance)) {
                throw new moodle_exception('wscannotunenrol', 'enrol_ishinemanual', '', $enrolment);
            }
