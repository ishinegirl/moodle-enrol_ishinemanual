 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_ishinemanual_lib_testcase extends advanced_testcase {
    /**
    ...
    public function test_migrate_plugin_enrolments() {
        ...
        /** @var $manplugin enrol_ishinemanual_plugin */
        $manplugin = enrol_get_plugin('ishinemanual');
        ...
        // We expect ishinemanual, self and guest instances to be created by default.

        $this->assertEquals(5, $DB->count_records('enrol', array('enrol'=>'ishinemanual')));
        ...
        // Enrol some users to ishinemanual instances.

        $maninstance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $DB->set_field('enrol', 'status', ENROL_INSTANCE_DISABLED, array('id'=>$maninstance1->id));
        $maninstance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $maninstance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $DB->delete_records('enrol', array('courseid'=>$course3->id, 'enrol'=>'ishinemanual'));
        $DB->delete_records('enrol', array('courseid'=>$course4->id, 'enrol'=>'ishinemanual'));
        $DB->delete_records('enrol', array('courseid'=>$course5->id, 'enrol'=>'ishinemanual'));
        ...
        enrol_ishinemanual_migrate_plugin_enrolments('xxx');

        // Verify results.

        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course1->id, 'enrol'=>'ishinemanual')));
        ...
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course2->id, 'enrol'=>'ishinemanual')));
        ...
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course3->id, 'enrol'=>'ishinemanual')));
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course3->id, 'enrol'=>'xxx')));
        $maninstance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        ...
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course4->id, 'enrol'=>'ishinemanual')));
        $this->assertEquals(2, $DB->count_records('enrol', array('courseid'=>$course4->id, 'enrol'=>'xxx')));
        $maninstance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        ...
        $this->assertEquals(0, $DB->count_records('enrol', array('courseid'=>$course5->id, 'enrol'=>'ishinemanual')));
        ...
        enrol_ishinemanual_migrate_plugin_enrolments('ishinemanual');
        enrol_ishinemanual_migrate_plugin_enrolments('yyyy');
    }
    ...
    public function test_expired() {
        ...
        /** @var $ishinemanualplugin enrol_ishinemanual_plugin */
        $ishinemanualplugin = enrol_get_plugin('ishinemanual');
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'ishinemanual')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance1->roleid);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance2->roleid);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance3->roleid);

        $this->assertEquals(0, $DB->count_records('user_enrolments'));
        $this->assertEquals(0, $DB->count_records('role_assignments'));

        $ishinemanualplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $ishinemanualplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $ishinemanualplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $ishinemanualplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $ishinemanualplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now+60*60);
        $ishinemanualplugin->enrol_user($instance3, $user3->id, $teacherrole->id, 0, $now-60*60);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $ishinemanualplugin->get_config('expiredaction'));
        $ishinemanualplugin->sync($trace, null);
        $this->assertEquals(6, $DB->count_records('user_enrolments'));
        $this->assertEquals(7, $DB->count_records('role_assignments'));


        $ishinemanualplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $ishinemanualplugin->sync($trace, $course2->id);
        $this->assertEquals(6, $DB->count_records('user_enrolments'));
        $this->assertEquals(7, $DB->count_records('role_assignments'));
        ...
        $ishinemanualplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
        ...
        $ishinemanualplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPEND);
        $ishinemanualplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);
        $ishinemanualplugin->enrol_user($instance3, $user3->id, $teacherrole->id, 0, $now-60*60);
        $maninstance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $maninstance2 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
    ...
    public function test_send_expiry_notifications() {
        ...
        $ishinemanualplugin->set_config('expirynotifylast', $now - 60*60*24);
        $ishinemanualplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'ishinemanual')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'ishinemanual'), '*', MUST_EXIST);
        $instance4->expirythreshold = 60*60*24*1;
        $instance4->expirynotify    = 0;
        $instance4->notifyall       = 0;
        $DB->update_record('enrol', $instance4);

        $ishinemanualplugin->enrol_user($instance1, $user1->id, $editingteacherrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $ishinemanualplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $ishinemanualplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $ishinemanualplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $ishinemanualplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $ishinemanualplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        $ishinemanualplugin->enrol_user($instance1, $user7->id, $editingteacherrole->id);
        $ishinemanualplugin->enrol_user($instance1, $user8->id, $managerrole->id);                                             // Highest role --> enroller.

        $ishinemanualplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $ishinemanualplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $ishinemanualplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $ishinemanualplugin->enrol_user($instance3, $user1->id, $editingteacherrole->id);
        $ishinemanualplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $ishinemanualplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $ishinemanualplugin->enrol_user($instance4, $user4->id, $editingteacherrole->id);
        $ishinemanualplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $ishinemanualplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $ishinemanualplugin->send_expiry_notifications($trace);
        ...
        $ishinemanualplugin->set_config('expirynotifylast', time() - 60*60*24);
        $ishinemanualplugin->set_config('expirynotifyhour', '24');
