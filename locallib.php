...
/**
 * Auxiliary ishinemanual user enrolment lib, the main purpose is to lower memory requirements...
 *
 ...
 * Enrol candidates.
 */
class enrol_ishinemanual_potential_participant extends user_selector_base {
    protected $enrolid;
    ...
    protected function get_options() {
        ...
        $options['file']    = 'enrol/ishinemanual/locallib.php';
        return $options;
    }
}
 ...
 * Enrolled users.
 */
class enrol_ishinemanual_current_participant extends user_selector_base {
    protected $courseid;
    protected $enrolid;
...
/**
 * A bulk operation for the ishinemanual enrolment plugin to edit selected users.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_ishinemanual_editselectedusers_operation extends enrol_bulk_enrolment_operation {
    ...
    public function get_title() {
        return get_string('editselectedusers', 'enrol_ishinemanual');
    }
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/ishinemanual:manage", $manager->get_context())) {
            return false;
        }
                ...
                foreach ($user->enrolments as $enrolment) {
                    ...
                    $event = \core\event\user_enrolment_updated::create(
                            array(
                                ...
                                'other' => array('enrol' => 'ishinemanual')
                                )
                            );
                    $event->trigger();
                }
     ...
     * @return enrol_ishinemanual_editselectedusers_form
     */
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        global $CFG;
        require_once($CFG->dirroot.'/enrol/ishinemanual/bulkchangeforms.php');
        return new enrol_ishinemanual_editselectedusers_form($defaultaction, $defaultcustomdata);
    }
}


/**
 * A bulk operation for the ishinemanual enrolment plugin to delete selected users enrolments.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_ishinemanual_deleteselectedusers_operation extends enrol_bulk_enrolment_operation {
     ...
     */
    public function get_title() {
        return get_string('deleteselectedusers', 'enrol_ishinemanual');
    }
    ...
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        ...
        $defaultcustomdata['message'] = get_string('confirmbulkdeleteenrolment', 'enrol_ishinemanual');
        $defaultcustomdata['button'] = get_string('unenrolusers', 'enrol_ishinemanual');
        return new enrol_ishinemanual_deleteselectedusers_form($defaultaction, $defaultcustomdata);
    }
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/ishinemanual:unenrol", $manager->get_context())) {
            return false;
        }
        foreach ($users as $user) {
            foreach ($user->enrolments as $enrolment) {
...
/**
 * Migrates all enrolments of the given plugin to enrol_ishinemanual plugin,
 ...
 */
function enrol_ishinemanual_migrate_plugin_enrolments($enrol) {
    global $DB;

    if ($enrol === 'ishinemanual') {
    ...
    $sql = "SELECT e.id, e.courseid, e.status, MIN(me.id) AS mid, COUNT(ue.id) AS cu
         ...
         LEFT JOIN {enrol} me ON (me.courseid = e.courseid AND me.enrol='ishinemanual')
             WHERE e.enrol = :enrol
    ...
    foreach($rs as $e) {
        $minstance = false;
        if (!$e->mid) {
            // iShineManual instance does not exist yet, add a new one.
            $course = $DB->get_record('course', array('id'=>$e->courseid), '*', MUST_EXIST);
            if ($minstance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'ishinemanual'))) {
                // Already created by previous iteration.
                $e->mid = $minstance->id;
            } else if ($e->mid = $ishinemanualplugin->add_default_instance($course)) {
        ...
        if (!$minstance) {
            ...
            debugging('Failed to find ishinemanual enrolment instance', DEBUG_DEVELOPER);
            continue;
        }
        ...
        // Migrate to ishinemanual enrol instance.
        $params = array('id'=>$e->id, 'mid'=>$e->mid);
 ...
 */
function enrol_ishinemanual_get_potential_cohorts($context, $enrolid, $search = '', $page = 0, $perpage = 25, $addedenrollment = 0) {
    global $CFG;
...
/**
 * Gets an array of timestamps or labels
 *
 * @param boolean $labels
 * @return array array of timestamps or timestamp labels(human readable)
 */
function enrol_ishinemanual_get_startdates($labels=true)
{
    $timestamps = array();
    $timelabels = array();
    ...
    for ($i = -1; $i < 12; $i++) {
        ...
        $useday = make_timestamp($useyear, $usemonth, 1, 0, 0, 0);
        $timestamps[] = $useday;
    }
    ...
    if (!$labels) {
        ...
        foreach($timestamps as $timestamp){
            $timelabels[] = userdate($timestamp, $dateformat);
        }
        return $timelabels;
    }
}